...
function init(){
    for(let i=0;i<n;i++){
        array[i] = Math.random(); //給每個column random 值 
    }
    moves = [];    
    for(let i=0;i<array.length;i++){
        const x = i*spacing + spacing/2 + margin; 
        const y = myCanvas.height - margin //-i*3 利用高度差 可以增加立體感
...
function animate(){
    ...
    for(let i=0;i<cols.length;i++){
        changed = cols[i].draw(ctx) || changed; //等上一個動作完再執行下一個
    }
